 ...
 */
function encodePacket(magazine: number, packetNumber: number, data: Uint8Array): Uint8Array {
...
function exportPagePackets(page: TeletextPage, subpageIdx: number = 0): Uint8Array[] {
  ...
  // Header packet
  packets.push(encodePacket(payload.headerPacket.magazine, 0, payload.headerPacket.payload));

  // Display packets (rows 1-23)
  for (const dp of payload.displayPackets) {
    packets.push(encodePacket(dp.magazine, dp.packetNumber, dp.payload));
  }
